...
public class LvlCmd extends XpCommand {
    ...
    private MessageEmbed genEmbed(net.dv8tion.jda.api.entities.User member, Guild guild) {
        List<User> users = new java.util.ArrayList<>(Main.context.
                retrieveGroup(new RetrieveGroupRequest(guild.getId())).membersAsList());
        users.sort(Collections.reverseOrder());
        User user = Main.context.retrieveMember(new RetrieveMemberRequest(member.getId(), guild.getId()));
        ...
        embed.addField("",
                MarkdownUtil.codeblock("java", progress((user.getXp() - Main.calculator.totalRequired(user.getLevel() - 1)) / Main.calculator.xpRequired(user.getLevel())) +
                        " (" + new DecimalFormat("#,###.##").format(user.getXp() - Main.calculator.totalRequired(user.getLevel() - 1)) + "/" + new DecimalFormat("#,###.##").format(Main.calculator.xpRequired(user.getLevel())) + ")"),
                false);
